...
It classifies recognized Claude, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, and Hermes process names as `alive`, common shells as `dead`, an authoritatively absent window as `missing`, unreadable state as `unreadable`, and every other process as `ambiguous`.
