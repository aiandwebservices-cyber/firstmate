...
| `fm-specialist-resolve.sh` | Resolve one wshobson/agents specialist from the catalog plus the on-disk name index |
...
| `fm-subagent-pretool-check.sh` | Primary-home delegation-shape PreToolUse guard, currently disabled (docs/subagent-guard.md) |
...
| `fm-zeus-worker.sh`      | Launch the local Hermes Agent coding worker on the Zeus DeepSeek stack for harness hermes or zeus |
