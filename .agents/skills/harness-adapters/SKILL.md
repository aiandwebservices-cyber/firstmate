...
| Fact | Value |
|---|---|
| Busy state | The one remaining rendered-tail fallback, isolated to Grok until its structured lifecycle is live-verified: mid-turn cancel hint in the keybind bar. Older grok used `Ctrl+c:cancel`; Grok 4.5 (live 2026-07-31) uses `Esc:cancel` while a turn is running. Idle bar omits that token (`Shift+Tab:mode │ Ctrl+x:shortcuts`). On herdr, native `agent_status=working` is also trusted as busy. ASCII cancel tokens are matched rather than the braille spinner to avoid locale fragility. |
...
| Interrupt | single `Ctrl+C` (cancels the current turn). Grok 4.5 also shows `Esc:cancel` mid-turn in the keybind bar; treat that footer token as busy-state evidence, not as a change to the interrupt key until re-verified. |
